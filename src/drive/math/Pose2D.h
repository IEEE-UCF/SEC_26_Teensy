...
Description:
Can be used to represent any type of value like position, acceleration, or
velocity. You will have to keep track of what type of value you are storing. For
example, you would have to use fixTheta() on a position vector, but you wouldn't
use it on a velocity vector. You can also chain together Pose2D as a lot of the
...
class Pose2D {
 public:
  ...
  /*
  Removed as using these functions results in bad overall workflow. Create a new
  Pose2D instead of setting these. Use reset and add if necessary void
  setX(float val) { x = val; } void setY(float val) { y = val; } void
  setThetaRadians(float val) { theta = val; } void setThetaDegrees(float deg) {
  theta = radians(deg); }*/
