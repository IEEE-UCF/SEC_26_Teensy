...
Description:
Declares a C++ template for creating highly flexible motor objects with
compile-time configuration of motor type (rotational/linear), physical
constants, pin assignments, and encoder type.
...
5/9/25 - Completed template structure with pin configs, constants, encoder
handling - Gemini 5/9/25 - Integrated external QuadEncoderHandler - Gemini
5/9/25 - Renamed methods to uppercase, changed speed input scale, added PWM
output range - Gemini
*/
...
// This header must define the QuadEncoderHandler class and QuadEncoderSetup
// struct
#include "../encoders/QuadEncoderHandler.h"

// --- Forward Declarations ---
// Forward declare the main Motor class template
template <typename MotorType, typename EncoderType, typename ConstantsType,
...
// --- Encoder Types ---
struct NoEncoder {};  // Type tag for no encoder
...
class AnalogEncoder {
 ...
 public:
  ...
  long GetPosition() const {
    return map(analogRead(analogPin), 0, 1023, 0, 1000);
  }
  void Reset() {}
};
...
struct NoMotorConstants {};  // Default for motors with no specific constants

struct RotationalMotorConstants {
  float noLoadRPM;
  float gearRatio;
  float maxSpeedInput;  // e.g., 1.0 for -1.0 to 1.0 range, or 255.0 for -255 to
                        // 255 range
  int maxPWMOut;        // e.g., 255 for 8-bit PWM

  constexpr RotationalMotorConstants(float rpm, float ratio, float maxInput,
                                     int maxPWM)
      : noLoadRPM(rpm),
        gearRatio(ratio),
...
struct LinearMotorConstants {
  ...
  constexpr LinearMotorConstants(float stroke, float speed_mm_s, float maxInput,
                                 int maxPWM)
...
struct TwoSpeedDir {
  uint8_t kSpeed;
  uint8_t kDir;
  constexpr TwoSpeedDir(uint8_t speedPin, uint8_t dirPin)
      : kSpeed(speedPin), kDir(dirPin) {}
};
...
struct ThreeSpeedDirDir {  // H-bridge style control with separate
                           // forward/reverse pins + speed
  ...
  constexpr ThreeSpeedDirDir(uint8_t speedPin, uint8_t dirAPin, uint8_t dirBPin)
      : kSpeed(speedPin),
        kDirA(dirAPin),
        kDirB(dirBPin) {}  // Corrected kDirBPB
};
struct FourSpeedDirDirEn {  // H-bridge style with enable
  ...
  uint8_t kDirB;
  uint8_t kEn;
  constexpr FourSpeedDirDirEn(uint8_t speedPin, uint8_t dirAPin,
                              uint8_t dirBPin, uint8_t enPin)
...
class Motor {
 public:
  ...
  // Constructor takes constants, pin configuration, and variadic args for
  // encoder initialization
  template <typename T_Constants, typename T_PinConfig, typename... EncoderArgs>
  Motor(T_Constants &&constants, T_PinConfig &&pin_config,
        ...
        encoder_storage_()  // Allocate raw storage
  {
    ...
    if constexpr (!std::is_same_v<EncoderType, NoEncoder>) {
      // Use perfect forwarding to pass the encoder_args to the EncoderType
      // constructor
  ...
  void Begin() {
    ...
    // Initialize motor state (e.g., set initial speed to 0, ensure disabled if
    // applicable)
  ...
  void Update() {
    if constexpr (!std::is_same_v<EncoderType, NoEncoder>) {
      ...
      if constexpr (std::is_same_v<EncoderType, QuadEncoderHandler>) {
        encoder_
            .UpdateEnc();  // Call the UpdateEnc method from QuadEncoderHandler
      }
      // Add calls for other specific encoder types if needed
    }
  ...
  void SetSpeed(float speed_value) {
    ...
    speed_value = constrain(speed_value, -constants_.maxSpeedInput,
  ...
  auto getEncoderPosition() -> std::enable_if_t<!std::is_same_v<T, NoEncoder>,
                                                long> const  // Added const
  {
    ...
    if constexpr (std::is_same_v<EncoderType, QuadEncoderHandler>) {
      ...
      static_assert(false,
                    "getEncoderPosition not implemented for this EncoderType");
  ...
  auto resetEncoder() -> std::enable_if_t<!std::is_same_v<T, NoEncoder>> {
    ...
    // Note: QuadEncoderHandler doesn't currently have a public reset method in
    // your .cpp Add logic here if you implement reset() in QuadEncoderHandler
    // or other encoder types.
    else {
 ...
 private:
  ...
  typename std::aligned_storage<sizeof(EncoderType), alignof(EncoderType)>::type
      encoder_storage_;
  // Reference to the object constructed via placement new
  // Note: This reference is only valid AFTER the constructor's placement new
  // call and BEFORE the destructor's explicit destructor call.
  ...
  int calculateMotorCommand(float speed_input) const {
    ...
    if constexpr (std::is_same_v<ConstantsType, RotationalMotorConstants> ||
      ...
      static_assert(ConstantsType::maxSpeedInput > 0,
                    "maxSpeedInput must be greater than 0");
      static_assert(
          ConstantsType::maxPWMOut > 0,
          "maxPWMOut must be greater than 0");  // Assuming 8-bit PWM max
    } else {
      // If ConstantsType doesn't have these members, maybe use defaults or
      // error
      static_assert(false,
                    "ConstantsType must define maxSpeedInput and maxPWMOut");
    }

    // Scale the input speed to the PWM output range
    // Handles negative speeds and ensures output is within [-maxPWMOut,
    // maxPWMOut]
    float scaled_command = (speed_input / constants_.maxSpeedInput) *
                           static_cast<float>(constants_.maxPWMOut);

    // Return as int, clamping to the valid integer range [-maxPWMOut,
    // maxPWMOut]
    return static_cast<int>(
        constrain(scaled_command, -static_cast<float>(constants_.maxPWMOut),
                  static_cast<float>(constants_.maxPWMOut)));
  }
  ...
  void WriteSpeed(int pwm_command) {
    ...
    if constexpr (std::is_same_v<PinConfigType, OneOnlySpeed>) {
      ...
      // simple PWM.
      analogWrite(pin_config_.kSpeed,
                  std::abs(pwm_command));  // Use std::abs for int
    } else if constexpr (std::is_same_v<PinConfigType, TwoSpeedDir>) {
      ...
      if (pwm_command >= 0) {  // Positive command for forward
        digitalWrite(pin_config_.kDir,
                     HIGH);  // Adjust HIGH/LOW based on wiring
      ...
      // Control for three-pin config (e.g., PWM + Digital Direction + Digital
      // Enable)
      uint8_t pwm_value = std::abs(pwm_command);  // Use std::abs for int
      if (pwm_value == 0) {
      ...
      if (pwm_command == 0) {
        ...
        digitalWrite(pin_config_.kDirB,
                     LOW);  // Coast or brake depending on driver
        analogWrite(pin_config_.kSpeed, 0);
