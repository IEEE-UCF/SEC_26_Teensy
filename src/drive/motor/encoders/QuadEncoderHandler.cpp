/*
QuadEncoderHandler.cpp - Interfacing class for using the Quadrature Encoder library
...
{
    if (!pinCheck(config.kENCA, validEncoderPins, numValidEncoderPins) || !pinCheck(config.kENCB, validEncoderPins, numValidEncoderPins))
    {
 ...
 */
long QuadEncoderHandler::GetEnc()
{
    return encoder ? enc : 0;
}
